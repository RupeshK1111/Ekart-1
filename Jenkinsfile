pipeline {
    ...
    stages {
        stage('git checkout') {
            steps {
                git branch: 'master', url: 'https://github.com/RupeshK1111/Ekart-1.git'
            }
        }
        ...
        stage('build and Tag docker image') {
            steps {
                script {
                        sh "docker build -t rupeshkorte1111/ekart-1:latest -f docker/Dockerfile ."
                    }
            }
        }
        ...
        stage('Push image to Hub'){
            steps{
                script{
                   ...
                   sh 'docker login -u rupeshkorte1111 -p ${dockerhubpwd}'}
                   sh 'docker push rupeshkorte1111/ekart-1:latest'
                }
            }
        }
